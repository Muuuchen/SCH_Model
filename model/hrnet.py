...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.dconv1 = DepthWiseConv(inplanes, planes)
        self.bn1 = nn.BatchNorm2d(planes, momentum=BN_MOMENTUM)
        self.relu = nn.ReLU(inplace=True)
        self.cbamnet = CBAMLayer(planes)
        self.dconv2 = DepthWiseConv(planes, planes)
    ...
    def forward(self, x):
        ...
        out = self.dconv1.forward(x)
        out = self.bn1(out)
        out = self.relu(out)
        out = self.cbamnet.forward(out)
        out = self.dconv2.forward(out)
        out = self.bn2(out)
